...
- [ ] `.github/workflows/release-tag.yml` promotes only the latest successful `main` SHA and advances `release`
...
- [ ] `bun run verify:repo` passes locally, CI still installs with a frozen Bun lockfile, and the release promotion workflow contract remains intact
...
- [ ] Dokploy is configured to deploy from `release`, not `main`
