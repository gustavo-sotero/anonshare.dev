...
- Keep infrastructure contracts aligned with the current baseline: a single root `.env` for local runtime and Docker Compose defaults, the same canonical variable names in CI and deployment, and `bun run infra:check` / `GET /health` as the readiness path.
